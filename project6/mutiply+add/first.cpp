...
main( int argc, char *argv[ ] )
{
    ...
    int LOCAL_SIZE = 1;
    int temp_NUM_ELEMENTS, temp_LOCAL_SIZE;
    int NUM_WORK_GROUPS;
    
	// see if we can even open the opencl kernel program
	// (no point going on if we can't):
    
    for (temp_NUM_ELEMENTS = NUM_ELEMENTS; temp_NUM_ELEMENTS <= NUM_ELEMENTS*1024*8; temp_NUM_ELEMENTS *= 4) {
        //the global work size range from 1k to 8M
        for (temp_LOCAL_SIZE = LOCAL_SIZE; temp_LOCAL_SIZE <= 1024; temp_LOCAL_SIZE *= 4) {
            FILE *fp;
#ifdef WIN32
            ...
            size_t dataSize = temp_NUM_ELEMENTS * sizeof(float);
            ...
            size_t localWorkSize[3]  = { temp_LOCAL_SIZE,   1, 1 };
            ...
            fprintf( stderr, "%8d\t%4d\t%10d\t%10.3lf GigaMultsPerSecond\n",
                    temp_NUM_ELEMENTS, temp_LOCAL_SIZE, NUM_WORK_GROUPS, (double)temp_NUM_ELEMENTS/(time1-time0)/1000000000. );

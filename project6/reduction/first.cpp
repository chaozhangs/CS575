...
//    int NUM_WORK_GROUPS;
    ...
    for (temp_NUM_ELEMENTS = NUM_ELEMENTS; temp_NUM_ELEMENTS <= NUM_ELEMENTS*1024*8; temp_NUM_ELEMENTS *= 4) {
        ...
        status = clSetKernelArg(kernel, 2, sizeof(float), NULL); // local ìprodsî array ñ one per work-item
        ...
        fprintf( stderr, "%8d\t%4d\t%10d\t%10.3lf GigaMultsPerSecond\n",
                temp_NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, (double)temp_NUM_ELEMENTS/(time1-time0)/1000000000. );
